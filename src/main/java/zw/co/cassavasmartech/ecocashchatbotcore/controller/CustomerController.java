...
public class CustomerController {
    ...
    @GetMapping("/{id}")
    public EntityModel<Customer> getCustomerByCustomerChatId(@PathVariable String id) throws NotFoundException {
        Customer customer = customerService.findByCustomerChatId(id).orElseThrow(()->new NotFoundException("Customer not found"));
    ...
    public ResponseEntity<?> updateActiveCustomers(@PathVariable String id) throws NotFoundException {
        Customer customer = customerService.findByCustomerChatId(id).orElseThrow(()->new NotFoundException("Customer not found"));
        if(customer)
    }
